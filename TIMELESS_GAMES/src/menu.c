   ...
   switch(event.type) {
      case SDL_MOUSEBUTTONUP:
 	      if (event.button.x < 337 && event.button.x > 106 && event.button.y < 272 && event.button.y > 83) {
            if(*mode_de_jeu == JVSO){
               init_partie_pendu(mode_de_jeu,win,ren,Joueur1->pseudo,Joueur2->pseudo);
               SDL_RenderPresent(ren);
            }else{
               init_partie_pendu(mode_de_jeu,win,ren,Joueur1->pseudo,Joueur2->pseudo);
               afficher_image("assets/pendu/pendujcj.png", 0, 0, win, ren);
               if(*etat_joueur ==J1)
                  afficher_texte("assets/inter.ttf", 27, 520, 80, Joueur1->pseudo, ren);
               else{
                  afficher_texte("assets/inter.ttf", 27, 520, 80, Joueur2->pseudo, ren);
               }
               SDL_RenderPresent(ren);
            }
            *etat_win = PENDU;
